...
KUBECONFIG=/root/.kube/config kubectl get po myredis -o jsonpath='{.spec.containers[*].livenessProbe.exec.command}' | grep -q -w "redis-cli PING"
...
KUBECONFIG=/root/.kube/config kubectl get po myredis -o jsonpath='{.spec.containers[*].readinessProbe.exec.command}' | grep -q -w "redis-cli PING"
